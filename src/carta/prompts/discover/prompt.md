...
1. Write approved discovery to temporary file `/tmp/discover-draft.md`

2. Execute save script:
   - **If GitHub issue link was provided:**
     ```bash
     bash src/carta/prompts/discover/scripts/save.sh "$USER_REQUIREMENT" "$BRANCH_NAME" /tmp/discover-draft.md "$ISSUE_NUMBER"
     ```
   - **Otherwise:**
     ```bash
     bash src/carta/prompts/discover/scripts/save.sh "$USER_REQUIREMENT" "$BRANCH_NAME" /tmp/discover-draft.md
     ```

3. Parse JSON output and confirm to user:
   "✓ Discovery saved to `[discover_path]` on branch `[branch_name]`"
